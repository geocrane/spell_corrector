...
_DIALOG_FONT = ("Segoe UI", 9)
_DIALOG_TITLE_FONT = ("Segoe UI", 10, "bold")


def _ask_snapshot_conflict(parent, doc_name: str) -> str:
    """Диалог выбора действия при существующем снимке.

    Returns: "update" | "new" | "cancel"
    """
    from ui.constants import RIBBON_BG, RIBBON_NORMAL_FG, RIBBON_HOVER_BG

    result_holder = ["cancel"]

    dlg = tk.Toplevel(parent)
    dlg.title("Снимок уже существует")
    dlg.resizable(False, False)
    dlg.grab_set()
    dlg.configure(bg=RIBBON_BG)

    outer = tk.Frame(dlg, bg=RIBBON_BG, padx=18, pady=14)
    outer.pack(fill=tk.BOTH, expand=True)

    tk.Label(
        outer,
        text=f"Снимок «{doc_name}» уже сохранён.",
        font=_DIALOG_TITLE_FONT, bg=RIBBON_BG, fg=RIBBON_NORMAL_FG,
        wraplength=300, justify="left",
    ).pack(anchor="w", pady=(0, 6))

    tk.Label(
        outer,
        text="Что сделать с существующим снимком?",
        font=_DIALOG_FONT, bg=RIBBON_BG, fg=RIBBON_NORMAL_FG,
    ...
    tk.Button(
        ...
        font=_DIALOG_FONT, relief="flat", padx=10, pady=5,
        bg="#007bff", fg="white",
        activebackground="#0056b3", activeforeground="white", cursor="hand2",
        command=lambda: _close("update"),
    ).pack(side=tk.LEFT, padx=(0, 6))

    tk.Button(
        btn_row, text="Создать новый",
        font=_DIALOG_FONT, relief="flat", padx=10, pady=5,
        bg="#28a745", fg="white",
        activebackground="#1e7e34", activeforeground="white", cursor="hand2",
        command=lambda: _close("new"),
    ).pack(side=tk.LEFT, padx=(0, 6))
    ...
    # Центрировать на родителе
    dlg.update_idletasks()
    px, py = parent.winfo_rootx(), parent.winfo_rooty()
    pw, ph = parent.winfo_width(), parent.winfo_height()
    dw, dh = dlg.winfo_reqwidth(), dlg.winfo_reqheight()
    dlg.geometry(f"+{max(0, px + (pw - dw)//2)}+{max(0, py + (ph - dh)//2)}")
...
class MainWindow(tk.Tk):
    ...
    def __init__(self, engine):
        ...
        # Режим отслеживания и сравнения документов
        self._tracking_mode = False         # toolbar переключён в режим отслеживания
        self._compare_result_doc = None     # документ, по которому показаны результаты
        self._compare_items: list = []      # результат compute_diff
        self._highlight_on = False          # жёлтая подсветка включена
        self._highlighted_ranges: list[tuple[int, int]] = []  # что именно подсвечено
    ...
    def _create_ui(self):
        ...
        # Кнопки режима отслеживания / сравнения
        self.tracking_mode_toggle = RibbonToggle(
            self.primary_row,
            icon_key="compare",
            text="Сравнение",
        ...
        self.compare_open_button = RibbonButton(
            self.primary_row,
            icon_key="compare",
            text="Сравнить",
        ...
        self.snapshots_button = RibbonButton(
            self.primary_row,
            icon_key="snapshots",
            text="Снимки",
        ...
        self.highlight_toggle = RibbonToggle(
            self.primary_row,
            icon_key="highlight",
            text="Подсветить",
            command=self._toggle_highlight_additions,
            icon_size=icons.ICON_MD,
            orient="vertical",
            size="medium",
            initial_state="off",
        ...
        # Toast-баннер для заметных уведомлений (создаётся скрытым).
        self.toast_label = tk.Label(
            self, font=("Segoe UI", 10, "bold"), fg="white", anchor="center",
            pady=6,
        )
    ...
    def _show_toast(self, message: str, color: str = "#28a745"):
        """Показать заметный цветной баннер на несколько секунд (без popup)."""
        if self._toast_after:
            self.after_cancel(self._toast_after)
        self.toast_label.config(text=f"✓  {message}", bg=color)
        self.toast_label.pack(
            fill=tk.X, padx=10, pady=(0, 4), before=self._main_container
        )
        self._toast_after = self.after(2800, self._hide_toast)
    ...
    def _update_button_state(self):
        ...
        if self.current_view == "documents":
            if self._tracking_mode:
                # Режим отслеживания: [Назад] [Документы] [Отслеживать] [Сравнить] [Снимки]
                self.back_button.set_command(self._toggle_tracking_mode)
                ...
                self.tracking_mode_toggle.pack(
    ...
    def _toggle_tracking_mode(self):
        """Включить / выключить режим отслеживания (toolbar переключается)."""
        self._tracking_mode = not self._tracking_mode
        self._update_button_state()
        if self._tracking_mode:
            self.status_label.config(
                text="Режим сравнения: «Отслеживать» — сохранить снимок, «Сравнить» — сравнить"
            )
        else:
            self._update_status_text()

    def _track_active_document(self):
        """Сохранить снимок текущего активного Word-документа."""
        doc = self.engine.selected_doc
        if doc is None:
            all_docs = self.engine.documents if hasattr(self.engine, "documents") else []
            word_docs = [d for d in all_docs if d.get("type") == "word"]
            if word_docs:
                doc = word_docs[0]
        if doc is None or doc.get("type") != "word":
            self.status_label.config(text="Нет открытого Word-документа для отслеживания")
            return

        doc_name = doc["name"]
        create_new = False

        if self._tracking_manager.has_snapshot(doc_name):
            answer = _ask_snapshot_conflict(self, doc_name)
            if answer == "cancel":
                return
            if answer == "new":
                create_new = True
        ...
        try:
            path = self._tracking_manager.save_snapshot(
                doc["com_object"], doc_name, create_new=create_new
            )
            import os
            display = os.path.splitext(os.path.basename(path))[0]
            self._show_toast(f"Снимок сохранён: {display}")
    ...
    def _open_compare_dialog(self):
        """Открыть диалог выбора документа и снимка для сравнения."""
        from ui.compare_dialog import CompareDialog

        all_docs = self.engine.documents if hasattr(self.engine, "documents") else []
        word_docs = [d for d in all_docs if d.get("type") == "word"]
        if not word_docs:
            self.status_label.config(text="Нет открытых Word-документов")
            return
        ...
        CompareDialog(
            ...
            on_compare=self._run_comparison,
        )

    def _open_snapshots_manager(self):
    ...
    def _run_comparison(self, doc: dict, snapshot_path: str):
        ...
        from core.comparison import (
            extract_paragraphs, extract_paragraphs_from_docx,
            compute_similarity_ratio, compute_diff,
        )

        try:
            paras_current = extract_paragraphs(doc["com_object"])
        except Exception as exc:
            self.status_label.config(text=f"Ошибка чтения документа: {exc}")
            return

        try:
            paras_snapshot = extract_paragraphs_from_docx(snapshot_path)
        except Exception as exc:
            self.status_label.config(text=f"Ошибка чтения снимка: {exc}")
            return

        if not paras_snapshot:
            self.status_label.config(text="Снимок пуст или не удалось прочитать")
            return

        # Проверка схожести — предупреждение если документы слишком разные
        ratio = compute_similarity_ratio(paras_snapshot, paras_current)
        if ratio < 0.12:
            pct = int(ratio * 100)
            proceed = messagebox.askyesno(
                "Документы очень различаются",
                f"Схожесть документов: {pct}%.\n"
                "Возможно, это два разных документа, а не версии одного.\n\n"
        ...
        # Снять подсветку предыдущего сравнения, если осталась
        if self._highlight_on and self._compare_result_doc is not None:
            self._clear_highlights(self._compare_result_doc)
        self._highlight_on = False
        self._highlighted_ranges = []

        items = compute_diff(paras_snapshot, paras_current)
        self._compare_items = items
        self._compare_result_doc = doc

        # Показать только текущий документ (без side-by-side)
        target_rect = (
            self.monitor["x"],
            self.monitor["y"],
        ...
        self._show_comparison_view(items, doc, n_paras=len(paras_current))

    def _show_comparison_view(self, items: list, doc: dict, n_paras: int = 0):
        ...
        self.canvas.yview_moveto(0)
        if not items:
            suffix = f" (проверено {n_paras} абзацев)" if n_paras else ""
            self.status_label.config(
                text=f"Документ идентичен снимку{suffix} — изменений нет"
            )
            return
        self.status_label.config(
            text=f"{doc['name']}: {len(items)} изменений относительно снимка"
        )
        for item in items:
    ...
    def _on_comparison_tile_click(self, diff_item: dict):
        """Клик по diff-плитке — навигация в текущем документе."""
        doc = self._compare_result_doc
        if doc is None:
            return
        tag = diff_item["tag"]
        if tag in ("replace", "insert") and diff_item["range_start_b"] is not None:
            office_finder.navigate_to_range(
                doc, diff_item["range_start_b"], diff_item["range_end_b"]
            )
        elif tag == "delete":
            # Нет абзаца в текущем документе — переходим к ближайшей якорной позиции
            start = diff_item.get("anchor_range_start_b")
            end = diff_item.get("anchor_range_end_b")
            if start is not None and end is not None:
                office_finder.navigate_to_range(doc, start, end)

    def _toggle_highlight_additions(self):
        """Включить / выключить жёлтую заливку добавленных мест в документе."""
        doc = self._compare_result_doc
        if doc is None:
            return

        if self._highlight_on:
            self._clear_highlights(doc)
            self._highlight_on = False
            self.highlight_toggle.set_state("off")
        else:
            self._apply_highlights(doc)
            self._highlight_on = True
            self.highlight_toggle.set_state("on")

    def _apply_highlights(self, doc: dict):
        """ON: подсветить добавленные фрагменты, запомнить их диапазоны.

        "insert"  — весь новый абзац(ы) целиком.
        "replace" — только изменённые слова через SequenceMatcher + Word Find.
        """
        self._highlighted_ranges = []
        for item in self._compare_items:
            rs = item.get("range_start_b")
            re = item.get("range_end_b")
            if rs is None or re is None:
                continue
            if item["tag"] == "insert":
                if office_finder.highlight_range(doc, rs, re, on=True):
                    self._highlighted_ranges.append((rs, re))
            elif item["tag"] == "replace":
                self._highlight_char_diff(
                    doc, rs, re,
                    item.get("text_a", ""),
                    item.get("text_b", ""),
                )

    def _clear_highlights(self, doc: dict):
        """OFF: снять заливку напрямую с запомненных диапазонов.

        Прямая операция Range.HighlightColorIndex=0 надёжно снимает заливку
        со всего диапазона (включая знаки абзаца), не требует Find.
        """
        for s, e in self._highlighted_ranges:
            office_finder.highlight_range(doc, s, e, on=False)
        self._highlighted_ranges = []
    ...
    def _highlight_char_diff(
        ...
        """Подсветить только добавленные/изменённые фрагменты, запомнив диапазоны.

        Использует Word Find — не зависит от смещений в Python-строке.
        Разбивает фрагменты по \\n (искусственный разделитель абзацев в text_b,
        которого нет в Word). Курсор движется вперёд после каждого найденного
        фрагмента, исключая повторное выделение.
        """
        import difflib as _difflib
        sm = _difflib.SequenceMatcher(None, old_text, new_text, autojunk=False)
        cursor = range_start
        for op, _, _, j1, j2 in sm.get_opcodes():
            if op in ("insert", "replace") and j2 > j1:
                fragment = new_text[j1:j2]
                for part in fragment.split("\n"):
                    if not part.strip():
                        continue
                    found = office_finder.highlight_text_fragment(
                        doc, cursor, range_end, part,
                    )
                    if found:
                        self._highlighted_ranges.append(found)
                        cursor = found[1]

    def _exit_comparison_view(self):
        """Выйти из режима просмотра результатов сравнения."""
        if self._highlight_on and self._compare_result_doc is not None:
            self._clear_highlights(self._compare_result_doc)
        self._highlight_on = False
        self._highlighted_ranges = []
        self._compare_items.clear()
        self._compare_result_doc = None
        self.highlight_toggle.set_state("off")
